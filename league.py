...
class League(object):
    ...
    def _fetch_owners(self):        
        ...
        for member in members:
            self.owners.append(Owner(member))
    ...
    def recent_activity(self, size: int = 25, only_trades = False) -> List[Activity]:
        ...
        if self.year < 2020:
            raise Exception('Cant use recent activity before 2020')
    ...
    def box_scores(self, week: int = None) -> List[BoxScore]:
        ...
        if self.year < 2020:
            raise Exception('Cant use box score before 2020')
    ...
    def free_agents(self, week: int=None, size: int=50, position: str=None) -> List[Player]:
        ...
        if self.year < 2020:
            raise Exception('Cant use free agents before 2020')
        if not week:
            week = self.current_week
